...
from .ocr_extract_agent import pedieat_image_extract_agent
...
class ClinicalContextGenerator:
    ...
    def __init__(self, model_name: str = "gpt-4o"):
        ...
        # )
        self.llm = init_chat_model(f"openai:{model_name}", temperature=0.7)
        self.setup_logging()
...
def pedieat_contextual_agent(
    pdf_path: str, /,
):
    """Main function to run the contextual interpretation agent"""
    
    output_file = "outputs/aws_pedieat_contextual.json"
    
    try:
        # json extract data
        input_data = pedieat_image_extract_agent(
            pdf_path, 
            amazon_access_key_id=config.AMAZON_ACCESS_KEY_ID, 
            amazon_secret_access_key=config.AMAZON_SECRET_ACCESS_KEY, 
            amazon_region=config.AMAZON_REGION
        )
        if not input_data:
            logger.info("Return empty {} from pedieat_image_extract_agent")
            raise RuntimeError("Unable to extract data from pedieat pdf.")
        ...
        # logger.info("🚀 Starting contextual interpretation workflow")
        ...
        # # Save the results
        # save_contextual_data(result["processed_data"], output_file)
        
        # logger.info(f"🎉 Contextual interpretation complete!")
        ...
        return {
            "success": True,
            "pedieat_data": input_data
        }
    
    except RuntimeError as e:
        logger.info("Return empty dict from pedieat image extract: {}".format(e))
        return {
            "success": False
        }
