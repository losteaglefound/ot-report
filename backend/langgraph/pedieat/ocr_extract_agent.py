...
def pedieat_image_extract_agent(
        pdf_path, /,
    ...
    try:
        ...
        logger.info(f"File to process: {pdf_path}")

        # Step 2: Get raw Textract responses for each page
        raw_responses = analyzer.get_raw_page_responses(pdf_path)
        logger.info("Generated raw response")

        # Step 3: Merge the responses into a single object
        merged_response = merge_textract_responses(raw_responses)
        logger.info("Response merged")
