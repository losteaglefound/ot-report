...
extraction_prompt_template = ChatPromptTemplate.from_messages([
    ("system", """You are a structured medical form parser. Your job is to read raw pediatric eating assessment data and extract it as a well-structured JSON format."""),
    ("human", """{prompt}
    ...
    "PhysiologicSymptoms": {{
        "items": [
        {{ "number": 1, "description": "gets watery eyes when eating", "score": 2 }},
        {{ "number": 2, "description": "gets red color around eyes or face when eating", "score": 0 }},
        {{ "number": 3, "description": "coughs during or after eating", "score": 2 }}
        ],
        "subscaleScore": 36
    }},
    "ProblematicMealtimeBehaviors": {{
        "items": [
        {{ "number": 28, "description": "avoids eating by playing or talking", "score": 0 }},
        {{ "number": 29, "description": "has to be told to start eating", "score": 1 }}
        ],
        "subscaleScore": 17
    }},
    "SelectiveRestrictiveEating": {{
        "items": [
        {{ "number": 51, "description": "will eat mixed texture foods", "score": 2 }}
        ],
        "subscaleScore": 43
    }},
    "OralProcessing": {{
        "items": [
        {{ "number": 66, "description": "stores food in their cheek or roof of mouth", "score": 1 }}
        ],
        "subscaleScore": 15
    }}
    }}
...
def parse_sensory_data(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        prompt = f"""
        Each score line will look like:
        Score1 gets watery eyes when eating: 2
        Score36 insists on food being offered in a certain way: 3
        ...
        - Full description
        - Score
        - Group items under their correct subscale, based on item numbers.
        - Include the subscale’s total score where available.
